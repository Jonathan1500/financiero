    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-ink-muted text-sm">Define objetivos y alcanza tus metas financieras</p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      {/* Summary */}
      {metas.length > 0 && (
        <div className="bg-gradient-to-r from-wealth to-wealth-light rounded-xl p-6 mb-6 text-white">
      ...
      {metas.length === 0 ? (
        <div className="bg-card rounded-xl border border-border py-16 text-center">
          <Target className="w-12 h-12 text-ink-subtle mx-auto mb-3" />
          <p className="text-ink-muted mb-2">No tienes metas de ahorro</p>
          <p className="text-sm text-ink-muted">Crea una meta para empezar a ahorrar</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {metas.map((meta) => {
            ...
            return (
              <div key={meta.id} className="bg-card rounded-xl border border-border p-5 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${completada ? "bg-wealth/10" : "bg-wealth/10"}`}>
                      <Target className={`w-4 h-4 ${completada ? "text-wealth" : "text-wealth"}`} />
                    </div>
                    <span className="font-medium text-ink">{meta.nombre}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openDepositarModal(meta, "deposito")} className="text-ink-subtle hover:text-wealth p-1" title="Depositar">
                      <ArrowDown className="w-4 h-4" />
                    </button>
                    <button onClick={() => openDepositarModal(meta, "retiro")} className="text-ink-subtle hover:text-amber p-1" title="Retirar">
                      <ArrowUp className="w-4 h-4" />
                    </button>
                    <button onClick={() => openModal(meta)} className="text-ink-subtle hover:text-wealth p-1">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(meta.id)} className="text-ink-subtle hover:text-danger p-1">
                ...
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-ink-muted">Ahorrado</span>
                ...
                <div className="w-full h-2 bg-surface rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${completada ? "bg-wealth" : "bg-wealth"}`}
                    style={{ width: `${progreso}%` }}
                  />
                </div>

                <div className="flex justify-between items-center mt-2">
                  <p className="text-xs text-ink-subtle">{progreso.toFixed(0)}% completado</p>
                  {diasRestantes !== null && (
                    <p className={`text-xs ${diasRestantes < 0 ? "text-danger" : "text-ink-subtle"}`}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-ink mb-1">Monto Objetivo</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
              ...
              {!editando && (
                <div>
                  <label className="block text-sm font-medium text-ink mb-1">Monto Inicial (opcional)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
              ...
              <div>
                <label className="block text-sm font-medium text-ink mb-1">Fecha Límite (opcional)</label>
                <input
                  type="date"
                  value={fechaLimite}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              ...
              <button onClick={() => setShowDepositarModal(false)} className="text-ink-muted hover:text-ink">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleDeposito} className="p-6 space-y-4">
              ...
              <button
                type="submit"
                className={`w-full py-3 text-primary-foreground font-medium rounded-lg transition-colors ${
                  tipoDeposito === "deposito"
                    ? "bg-wealth hover:bg-wealth-light"
